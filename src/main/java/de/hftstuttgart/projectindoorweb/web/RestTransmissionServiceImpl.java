...
public class RestTransmissionServiceImpl implements RestTransmissionService {
    ...
    public String processEvaalFiles(String buildingIdentifier, boolean evaluationFiles,
        ...
        if (transformedPointsFiles == null || transformedPointsFiles.length == 0) {
            ...
            return processEvaalFilesWithTransformedPoints(buildingIdentifier, evaluationFiles,
                    radioMapFiles, transformedPointsFiles);
        }
    }
        ...
        try {
            ...
            if (evaluationFile != null && TransmissionHelper.areRequestedFilesPresent(radioMapFiles)) {
                ...
                if(algorithmType == null || algorithmType.isEmpty()){
                    algorithmType = ConfigContainer.ALGORITHM_TYPE;
                }
                Set<SaveNewProjectParameters> saveNewProjectParameters = generateBatchPositionResults.getProjectParameters();
                Long evalFileId = evaluationFile.getId();
                long projectId = -1;
                boolean autoGeneratedProject = false;
                if (buildingId > 0) {

                    String projectName = String.format("AutoGenerated_%s", TransmissionHelper.getFormattedNowTimestamp());
                    GenericResponse genericResponse = this.persistencyService.createNewProject(projectName, algorithmType, saveNewProjectParameters, buildingId, evalFileId, radioMapFileIds);
                    if (genericResponse != null) {
                        projectId = genericResponse.getResultId();
                    }
                    autoGeneratedProject = true;
                } else {
                    projectId = generateBatchPositionResults.getProjectIdentifier();
                }

                if (projectId > -1) {
                    Project projectToUse = this.persistencyService.getProjectById(projectId);

                    if (projectToUse != null) {
                        List<WifiPositionResult> retrievedWifiResults =
                                (List<WifiPositionResult>) this.positionCalculatorService.
                                        calculatePositions(building, projectToUse,
    ...
    public SinglePositionResult getPositionForWifiReading(GenerateSinglePositionResult generateSinglePositionResult) {
        ...
        try {
            ...
            boolean autoGeneratedProject = false;
            if (algorithmType != null && !algorithmType.isEmpty()
                    && saveNewProjectParameters != null && !saveNewProjectParameters.isEmpty()
                    && buildingId > 0) {
                ...
                projectId = generateSinglePositionResult.getProjectIdentifier();
            }

            if (projectId > -1) {
                Project projectToUse = this.persistencyService.getProjectById(projectId);

                if (projectToUse != null && TransmissionHelper.areRequestedFilesPresent(radioMapFiles)) {
                    WifiPositionResult retrievedWifiResult = (WifiPositionResult) this.positionCalculatorService
                            .calculateSinglePosition(building, projectToUse,
    ...
    @Override
    public ResponseWrapper addNewProject(AddNewProject addNewProject) {

        if (addNewProject == null) {
            return TransmissionHelper.createExternalSimpleResponseWrapper(-1, ResponseConstants.PROJECT_GENERAL_FAILURE_NO_DATA_RECEIVED);
        }

        return TransmissionHelper.convertToExternalSimpleResponseWrapper(
                this.persistencyService.createNewProject(addNewProject.getProjectName(),
    ...
    @Override
    public ResponseWrapper addNewBuilding(AddNewBuilding addNewBuilding) {

        if (addNewBuilding == null) {
            return TransmissionHelper.createExternalSimpleResponseWrapper(-1, ResponseConstants.BUILDING_GENERAL_FAILURE_NO_DATA_RECEIVED);
        }
        ...
        try {
            ...
            return TransmissionHelper.convertToExternalSimpleResponseWrapper(
                    this.persistencyService.addNewBuilding(buildingName, numberOfFloors, imagePixelWidth, imagePixelHeight,
                            southEastAnchor, southWestAnchor, northEastAnchor, northWestAnchor, buildingCenterPoint,
                            rotationAngle, metersPerPixel));

        } catch (NumberFormatException ex) {
            return TransmissionHelper.createExternalSimpleResponseWrapper(-1, ResponseConstants.GENERAL_FAILURE_UNPARSABLE_NUMBER);
        }
    }
    ...
    private String processEvaalFilesWithTransformedPoints(String buildingIdentifier, boolean evaluationFiles,
        ...
        List<File> referencePointFiles = TransmissionHelper.convertMulipartFileBatchToLocalFileBatch(transformedPointsFile);
        ...
        try {
            ...
            if (building != null) {
                List<EvaalFile> processedEvalFiles = this.preProcessingService
