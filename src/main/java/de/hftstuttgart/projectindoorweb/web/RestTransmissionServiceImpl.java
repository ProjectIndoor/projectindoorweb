...
public class RestTransmissionServiceImpl implements RestTransmissionService {
    ...
    public List<BatchPositionResult> generatePositionResults(GenerateBatchPositionResults generateBatchPositionResults) {
        ...
        try {
            ...
            if (building != null && evaluationFile != null && TransmissionHelper.areRequestedFilesPresent(radioMapFiles)) {
                ...
                Project projectToUse = retrieveProjectToUse(generateBatchPositionResults.getProjectIdentifier(),
                        buildingId,
                        evaluationFile.getId(),
                        radioMapFileIds,
                        generateBatchPositionResults.getProjectParameters());

                if (projectToUse.getProjectName().toLowerCase().contains("autogenerated_")) {
                    autoGeneratedProject = true;
                }

                /*Use algorithmType here to call different kinds of PositionCalculationServices.
                 * Currently, only WifiPositionCalculation is available, thus there are no if statements
                 * deciding which service to use based on the value of algorithmType.*/
                ...
                if (autoGeneratedProject) {
                    this.persistencyService.deleteProject(projectToUse.getId());
                }
    ...
    public SinglePositionResult getPositionForWifiReading(GenerateSinglePositionResult generateSinglePositionResult) {
        ...
        SinglePositionResult result = TransmissionHelper.createEmptySinglePositionResult();
        ...
        try {
            ...
            if (building != null && TransmissionHelper.areRequestedFilesPresent(radioMapFiles)) {
                ...
                Project projectToUse = retrieveProjectToUse(generateSinglePositionResult.getProjectIdentifier(),
                        buildingId,
                        evalFileId,
                        radioMapFileIds,
                        generateSinglePositionResult.getSaveNewProjectParameters());
    ...
    public LoadSelectedProject loadSelectedProject(String projectIdentifier) {
        ...
        if (AssertParam.isNullOrEmpty(projectIdentifier)) {
            return TransmissionHelper.createEmptyProjectElement();
        }
    ...
    public List<GetAllProjects> getAllProjects() {
        ...
        return TransmissionHelper.convertToProjectElements(projects);
    ...
    public GetSingleBuilding getSingleBuilding(String buildingIdentifier) {
        ...
        GetSingleBuilding result = TransmissionHelper.createEmptySingleBuildingResult();
    ...
    private Project retrieveProjectToUse(long givenProjectId,
                                         long buildingId,
                                         long evalFileId,
                                         long[] radioMapFileIds,
                                         Set<SaveNewProjectParameters> saveNewProjectParameters) {

        Project projectToUse = null;

        boolean defaultProjectNeeded = false;
        if (saveNewProjectParameters == null || saveNewProjectParameters.isEmpty()) {

            projectToUse = this.persistencyService.getProjectById(givenProjectId);

            if (projectToUse != null) {
                return projectToUse;
            } else {
                defaultProjectNeeded = true;
            }

        }

        String projectName = String.format("AutoGenerated_%s", TransmissionHelper.getFormattedNowTimestamp());
        String algorithmType = ConfigContainer.ALGORITHM_TYPE;

        if (defaultProjectNeeded) {
            saveNewProjectParameters = TransmissionHelper.getDefaultParameterSet();
        }

        GenericResponse genericResponse = this.persistencyService.createNewProject(projectName, algorithmType,
                saveNewProjectParameters, buildingId, evalFileId, radioMapFileIds);
        projectToUse = this.persistencyService.getProjectById(genericResponse.getResultId());
